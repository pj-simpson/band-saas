...
class TestCreatingAnOrder(FunctionalTest):
    ...
    def test_can_add_items_to_shopping_basket_and_create_order(self):
        ...
        product_header = WebDriverWait(self.browser, timeout=4).until(
            lambda d: d.find_element_by_tag_name("h1")
        )
        ...
        checkout_link = WebDriverWait(self.browser, timeout=4).until(
            lambda d: d.find_element_by_id("checkout-link")
        )
        checkout_link.click()
        ...
        checkout_link = WebDriverWait(self.browser, timeout=4).until(
            lambda d: d.find_element_by_id("order-form")
        )
        ...
        place_order_button = self.browser.find_element_by_xpath(
            '// *[ @ id = "order-form"] / p[7] / input'
        )
        ...
        self.browser.switch_to.frame(
            self.browser.find_element_by_name("braintree-hosted-field-number")
        )
        ...
        self.browser.switch_to.frame(
            self.browser.find_element_by_name("braintree-hosted-field-cvv")
        )
        ...
        self.browser.switch_to.frame(
            self.browser.find_element_by_name("braintree-hosted-field-expirationDate")
        )
        expiration_date = self.browser.find_element_by_id("expiration")
        expiration_date.send_keys("1228")
        self.browser.switch_to_default_content()

        payment_button = self.browser.find_element_by_xpath(
            '//*[@id="payment"]/input[3]'
        )
        payment_button.click()

        # the payment form is submitted correctly to braintree and we are redirected back to a success page.
        success = WebDriverWait(self.browser, timeout=10).until(
            lambda d: d.find_element_by_id("success-message")
        )
