...
class FunctionalTest(StaticLiveServerTestCase):
    ...
    def confirm_homepage(self):
        home_link_class = self.browser.find_element_by_css_selector(
    ...
    def confirm_element_after_navigation(self, tag, text):
        header = WebDriverWait(self.browser, timeout=4).until(
            lambda d: d.find_element_by_tag_name(f"{tag}")
        )
        self.assertEqual(header.text, text)

    def confirm_nav_context(self, nav_text):
        active_link_class = self.browser.find_element_by_css_selector(
            "li.nav-item.active a"
        )
    ...
    def check_quantity_already_picked(self, value):
        quantity_selection = self.browser.find_element_by_xpath(
            f'//*[@id="id_quantity"]/option[{value}]'
        )
