...
def product_detail_view(request, slug):
    ...
    return render(
        ...
        {"nav": "shop", "product": product, "item_quantity_form": item_quantity_form},
    )
...
def basket_detail_view(request):
    cart = Cart(request)
    for item in cart:
        item["update_quantity_form"] = ItemQuantityForm(
            initial={"quantity": item["quantity"], "override": True}
        )
    return render(
        request, "shop/basket_detail.html", {"nav": "shop", "shopping_cart": cart}
    )
